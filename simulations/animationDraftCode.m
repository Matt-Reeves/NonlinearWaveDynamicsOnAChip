...
%load ./data/VdW/d0.5/Ac0.001.mat
...
x = [0; solution(1:simparams.N/2-1); pi]; 
y = solution(simparams.N/2:simparams.N); 
phi = solution(end)*ones(1,simparams.N);
T = solution(end-1);
...
simparams.dt     = simparams.DT/1200/4;                  %fine time step
...
%%
fracEnergyDeviation = max(abs(energy.E-energy.E(1))/energy.E(1))
energyDeviation = max(abs(energy.E-energy.E(1)))
...
clear height omega Acvals inds flagvec 
...
%Acvals = [0.00001:0.00001:0.00033 0.0004:0.0001:0.0009 0.001:0.001:0.191]
%Acvals = [0.0002:0.0002:0.304]
%Acvals = [0.012:0.002:0.188]
d = dir;
d = d(3:end);
N2 = 2*simparams.N; 
%simparams.N = 16;
for jj = 1:length(d)
    ...
    omega(jj)  = 2*pi/T;
    flagvec(jj) = EXITFLAG;
    Acvals(jj) = simparams.Act;
end

figure(1)

inds = (flagvec == 1  | flagvec == 3 );
subplot(121)
hold on
plot(Acvals(inds),omega(inds)/simparams.omega0,'.')
subplot(122)
hold on
plot(height(inds),omega(inds)/simparams.omega0,'.')
